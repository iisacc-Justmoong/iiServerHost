...
SDK는 Bonjour/NSD나 계정 로그인을 소유하지 않는다. Society 앱이 계정 범위의 기기 탐색·선택·초대 수신을 담당하고, SDK는 기존 TLS·Files 연결을 재사용한다. `iiServerHost.lan`은 선택 기기 바인딩, 최종 확인 전 Files 핸들러 호출 금지, 코드 일치, 확인·취소를 검사한다. 로그인 테스트는 호출하지 않는다.
